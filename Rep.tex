...
\begin{itemize}
  ...
  \item Pour résoudre 1, on utilise la méthode d'euler explicite :\\
  On choisit un pas $pas$, et, pour $i$ allant de $0$ à $T$ :

  \begin{equation*}
    \left\{\begin{array}{l}
             f(0) = x_0\\
             f((i+1).pas) = f(i)+pas.f'(i)
           \end{array}
    \right.\\
  \end{equation*}

  \item Pour résoudre 2, on utilise la même méthode, mais, en quelques sortes, dans le sens opposé :\\
        On choisit un pas $pas$, et, pour $i$ allant de $T$ à $0$ :
  \begin{equation*}
    \left\{\begin{array}{l}
             f(T) = x_T\\
             f((i-1).pas) = f(i)-pas.f'(i)
